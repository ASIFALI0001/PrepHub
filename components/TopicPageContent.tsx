...
const FREQ_STAT = [
  { key: "MOST_FREQ", label: "Must Know",  dot: "bg-orange-400" },
  { key: "MED",       label: "Expected",   dot: "bg-yellow-400" },
  { key: "LOW",       label: "Deep Dive",  dot: "bg-accent-blue" },
];
...
export default function TopicPageContent({ topic, initialLearnedIds }: Props) {
  ...
  useEffect(() => {
    fetch(`/api/content/learn/${topic.id}`)
      .then((r) => r.json())
      .then((d) => {
        if (d.error) { setHasContent(false); }
        else { setData(d); }
      })
  ...
  const handleReset = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        console.error("Reset failed:", await res.json().catch(() => ({})));
      }
    } catch (err) {
    ...
    <div>
      ...
      <div className="glass-card rounded-2xl border border-bg-border p-6 sm:p-8 mb-6">
        <div className="flex flex-col sm:flex-row sm:items-start gap-5 mb-6">
          <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 ${topic.iconBg}`}>
            {Icon && <Icon className={`w-7 h-7 ${topic.color}`} />}
          </div>
          <div className="flex-1">
            <h1 className="text-2xl font-bold text-text mb-1">{topic.label}</h1>
            <p className="text-text-muted leading-relaxed text-sm">{topic.desc}</p>
          </div>
        </div>
        ...
        <div className="mb-4">
          <div className="flex justify-between items-center mb-2">
            ...
            <span className={`text-sm font-bold ${pct > 0 ? topic.color : "text-text-muted"}`}>
              {pct}%
            </span>
          </div>
          ...
          {showResetConfirm ? (
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={handleReset}
                disabled={resetting}
                className="text-xs px-3 py-1.5 rounded-lg bg-red-500/15 text-red-400 hover:bg-red-500/25 font-medium"
              >
                {resetting ? "Resetting…" : "Yes, reset"}
              </button>
              <button
                onClick={() => setShowResetConfirm(false)}
                className="text-xs px-3 py-1.5 rounded-lg bg-bg-border text-text-muted hover:text-text"
              >
                Cancel
              </button>
            </div>
      ...
      {loading ? (
        ...
        <div className="space-y-5">
          ...
          <div className="glass-card rounded-2xl border border-bg-border p-5">
            <div className="flex flex-wrap gap-x-6 gap-y-3 mb-4">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <div className="text-base font-bold text-text">{data.metadata.total_questions}</div>
                  <div className="text-[10px] text-text-muted leading-none">Questions</div>
                </div>
              </div>
              {FREQ_STAT.map(({ key, label, dot }) => (
                <div key={key} className="flex items-center gap-2">
                  <div className={`w-2.5 h-2.5 rounded-full ${dot}`} />
                  <div>
                    <div className="text-base font-bold text-text">{data.summary.by_frequency[key]}</div>
              ...
              <div className="flex items-center gap-2 ml-auto">
                ...
                <div>
                  <div className="text-base font-bold text-accent-green">{learnedSet.size}</div>
                  <div className="text-[10px] text-text-muted leading-none">Learned</div>
                </div>
              </div>
            </div>
          ...
          {/* Study plan toggle */}
          <div className="glass-card rounded-xl border border-bg-border overflow-hidden">
            <button
              onClick={() => setShowStudyPlan((v) => !v)}
              className="w-full flex items-center justify-between px-5 py-3.5 hover:bg-bg-border/20 transition-colors"
            >
              <div className="flex items-center gap-2 text-sm font-medium text-text">
                <Star className="w-4 h-4 text-yellow-400" />
                Study Plan — where to start based on experience
              </div>
              <ChevronDown className={`w-4 h-4 text-text-muted transition-transform ${showStudyPlan ? "rotate-180" : ""}`} />
            </button>
            {showStudyPlan && (
              <div className="border-t border-bg-border px-5 py-4 space-y-3">
                {Object.entries(data.summary.study_plan).map(([key, val]) => {
                  ...
                  return (
                    <div key={key}>
                      <div className="text-xs font-semibold text-primary mb-0.5">{labels[key] ?? key}</div>
                      <div className="text-xs text-text-muted">{val}</div>
                    </div>
                  );
                })}
                <div className="pt-1 border-t border-bg-border">
                  <div className="text-xs font-semibold text-text mb-1.5">Highest probability topics</div>
                  <div className="flex flex-wrap gap-1.5">
                    {data.summary.highest_probability_topics.map((t) => (
                      <span key={t} className="text-[10px] px-2 py-0.5 rounded bg-primary/10 text-primary border border-primary/20">
                        {t}
                      </span>
                    ))}
                  </div>
          ...
          <div className="glass-card rounded-xl border border-bg-border overflow-hidden">
            ...
            >
              ...
              <ChevronDown className={`w-4 h-4 text-text-muted transition-transform ${showSources ? "rotate-180" : ""}`} />
            </button>
            {showSources && (
              <div className="border-t border-bg-border px-5 py-4 grid sm:grid-cols-2 gap-4">
                  ...
                  <div key={key} className="flex gap-3">
                    ...
                    <div>
                      <a
                        ...
                        className="text-xs font-semibold text-text hover:text-primary transition-colors flex items-center gap-1"
                      >
                        {src.full_name}
                        <ExternalLink className="w-3 h-3" />
                      </a>
                      <p className="text-[11px] text-text-muted mt-0.5 leading-relaxed">{src.strength}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {sections.map(([sectionName, questions]) => (
            <div key={sectionName} className="space-y-2">
              ...
              <div className="flex items-center justify-between px-1 mb-3">
                <h2 className="text-sm font-bold text-text">{sectionName}</h2>
                <span className="text-xs text-text-muted">
                  {questions.filter((q) => learnedSet.has(q.id)).length}/{questions.length} learned
                </span>
              </div>
